...
int main(){ // 6
	{
		...
		} return hEvStart_; }(), CloseHandle }; // 7

		OrCout.Trigger(true); // 8

		OrderLock lock; // 9

		thread t1(threadFunc, ref(lock), ref(OrCout), 1); // 9
		thread t2(threadFunc, ref(lock), ref(OrCout), 2); // 9
		thread t3(threadFunc, ref(lock), ref(OrCout), 3); // 9

		Sleep(200); // 10
		SetEvent(hEvStart_); // 11
